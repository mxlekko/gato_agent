...
AG-11 已建立无 OpenClaw 回归入口：`npm run regression:no-openclaw`，并完成本轮复验。`payment-info-split` 作为 direct-model 场景仍优先要求成功；若外部模型 provider 返回明确的 direct-model 失败，也按 external warning 处理，避免把模型服务过载误判为 OpenClaw 退场失败。
...
| `npm run regression:no-openclaw` | 通过；最近输出目录 `tests/regression/output/self-contained-2026-05-06T07-12-11-590Z`，5 个用例，4 pass，1 warning，0 fail；`OPENCLAW_GATEWAY_TOKEN_SET=false`，no-openclaw blocker 和日志检查均通过 |
...
- `payment-info-split` 和 `special-custom-product-solution` 仍依赖外部 direct-model provider；无 OpenClaw 回归允许明确的外部 provider warning，但不允许 OpenClaw blocker、legacy fallback 或 gateway-http 日志痕迹。
- `special-custom-product-solution` 仍依赖本地 RAG；无 OpenClaw 回归允许它以明确 warning 形式通过。
